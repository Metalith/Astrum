...
float rand(float co) {
	...
	float b = 78.233;
	float c = 43758.5453;
	float dt= dot(vec2(co, 1.0) ,vec2(a,b));
...
void main() {
	...
	vec3 modCoord = vec3(pos.xy  / 1024.0, 0.0);
	...
	float px = noise(modCoord + vec3(0.001 * rand(fract(time)), 0.0, 0.0), 8, 5, 0.5) - noise(modCoord - vec3(0.001 * rand(fract(time)), 0.0, 0.0), 8, 5, 0.5);
	float py = noise(modCoord + vec3(0.0, 0.001 * rand(fract(time)), 0.0), 8, 5, 0.5) - noise(modCoord - vec3(0.0, 0.001 * rand(fract(time)), 0.0), 8, 5, 0.5);
	// + bandfactor *
    //     cos((float) y / (float) ydim * numbands * 3.141527);
	color = vec4(px * 10.0, py * 10.0, 1.0, 1.0);
	// color = vec4(sign(px), sign(py), 1.0, 1.0);
}
