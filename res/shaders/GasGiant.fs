...
void main()
{
	vec3 tPos = pos + vec3(uTime * 5, 0.0, uTime * 5);
	float texCoord = tPos.y; // y is up
	vec3 lightDir = -normalize(vec3(1, 5, -5));
	float d = dot(vertexNormal, -lightDir);
	d = max(0.1, d);
	// float n = noise(pos, 5, 0.1, 0.8);
	float s = 0.7;
float t1 = snoise(tPos / 32 * 2.0) - s;
float t2 = snoise((tPos / 32 + 800.0) * 2.0) - s;
float t3 = snoise((tPos / 32  + 1600.0) * 2.0) - s; // Intersect them and get rid of negatives
float threshold = max(t1 * t2 * t3, 0.0); // Add to red color channel for debugging
float n1 = noise(tPos, 6, 0.1, 0.8);
	float n2 = ridgedNoise(tPos, 5, 0.2, 0.75);
