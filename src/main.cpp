...
#include "systems/ChunkManager.hpp"
...
ChunkManager cm = ChunkManager(vertices, normals, indices, bounds);
int main() {
	...
	for (auto chunk : ChunkList) {
		...
		std::cout << 100 * float(++p) / float(ChunkList.size()) << "%" <<  " " << vertices.size() / 3 <<  std::endl;
	}
	Draw();
...
bool CreateWindow() {
	...
	window = glfwCreateWindow( 1024, 768,("Astrum Engine Ver 0.2c." + UpdateVersion() + " - Luna").c_str(), NULL, NULL);
	if( window == NULL ){
...
void Draw() {
	...
	glBufferData(GL_ARRAY_BUFFER, normals.size() * sizeof(GLfloat), &normals[0], GL_STREAM_DRAW);
