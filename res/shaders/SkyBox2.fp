...
vec3 uSunPos = vec3(0, 1, 0);
uniform vec3 uCamPos;

#define M_PI 3.1415926535897932384626433832795

float TimeOfDay = 0.0; // range 0.0 -> 1.0 (0.0 = Midnight, 0.5 = Midday, etc)

const float RADIUS_EARTH = 3200e3;
const float RADIUS_ATMOSPHERE = 3232e3;
...
Sphere SPHERE_EARTH      = Sphere( vec3( 0 ), RADIUS_EARTH );
Sphere SPHERE_ATMOSPHERE = Sphere( vec3( 0 ), RADIUS_ATMOSPHERE );
...
bool intersect( in Ray ray, in Sphere sphere, out float t0, out float t1 )
{
   ...
   float q;
    if (b < 0.0)         q = (-b - sqrt(disc))/2.0;     else         q = (-b + sqrt(disc))/2.0;       t0 = q;     t1 = c / q;     // make sure t0 is smaller than t1     
...
void main()
{
    ...
    float z2 = x * x + y * y; 
        ...
        dir = normalize(pos);
        vec3 pos = uCamPos * 1e5;

        color = vec4( computeIncidentLight( Ray( pos, normalize( dir ) ) ), 1 );
}